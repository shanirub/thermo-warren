...
/* Icons are written as ASCII-art grids rather than packed bytes, following
 * the same reasoning as the font table above: the shape stays readable and
 * editable in source, and the bit arithmetic stays out of the error-prone
 * path. Costs a few dozen bytes of rodata per icon, which is the right trade
 * for something nobody can eyeball in hex. */
static void fb_draw_icon(int x0, int y0, const char *const rows[], int w, int h)
{
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            if (rows[y][x] == '#') {
                fb_set_pixel(x0 + x, y0 + y);
            }
        }
    }
}

/* Diagonal strike, bottom-left to top-right at 45 degrees, used to cross out
 * the Wi-Fi icon when the link is down.
 *
 * Two passes, and the order matters: every halo pixel is cleared before any
 * line pixel is set. Done in one pass, a later point's halo would erase the
 * line pixel its predecessor just drew, leaving a dashed strike.
 *
 * The halo is what makes this legible at all. The arcs are one pixel thick,
 * so a bare line drawn over them merges into the pattern and reads as noise;
 * clearing a pixel all round the strike interrupts the arcs visibly, which is
 * how a struck-through glyph is supposed to look. Verified by rendering both
 * states offscreen before flashing, not by guessing at the bitmap. */
static void fb_draw_slash(int x0, int y0, int w, int h)
{
    for (int k = 0; k <= h && (k + 1) < w; k++) {
        int x = x0 + 1 + k;
        int y = y0 + (h - 1) - k;
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                fb_clear_pixel(x + dx, y + dy);
            }
        }
    }
    for (int k = 0; k <= h && (k + 1) < w; k++) {
        fb_set_pixel(x0 + 1 + k, y0 + (h - 1) - k);
    }
}
...
/* Link marker: the usual three-arc Wi-Fi fan with a dot at the apex, struck
 * through with a diagonal when the broker session is down. A drawn icon rather
 * than a word because this stage's font covers only the characters
 * "Temp"/"Humidity" need, so a text status would mean extending the font table.
 *
 * The arcs were generated by a one-off script — points within 0.6px of radius
 * 3, 6 and 9 from the apex, inside a ~120 degree cone — and the output pasted
 * here, the same approach the font table used and for the same reason: hand
 * placing pixels to approximate three concentric arcs is exactly the kind of
 * thing that comes out subtly wrong and stays wrong.
 *
 * 15x10 rather than something smaller because a 1px strike through finer arcs
 * is illegible; two earlier attempts at 11x9 were discarded for that. There is
 * room: the top-right corner is empty out to x=96 on the lower text row and
 * x=72 on the upper one. */
#define WIFI_ICON_W 15
#define WIFI_ICON_H 10

static const char *const WIFI_ICON[WIFI_ICON_H] = {
    "....#######....",
    "..###.....###..",
    ".##.........##.",
    "##...#####...##",
...
#define LINK_ICON_X (OLED_WIDTH - WIFI_ICON_W)
#define LINK_ICON_Y 1
...
esp_err_t oled_show_readings(float temp_c, float humidity_pct, bool link_up)
{
    ...
    fb_draw_icon(LINK_ICON_X, LINK_ICON_Y, WIFI_ICON, WIFI_ICON_W, WIFI_ICON_H);
    if (!link_up) {
        fb_draw_slash(LINK_ICON_X, LINK_ICON_Y, WIFI_ICON_W, WIFI_ICON_H);
    }
